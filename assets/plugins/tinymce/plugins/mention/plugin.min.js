...
(function(f) {
	...
	AutoComplete.prototype = {
		...
		matcher: function(item) {
			if (typeof item[this.options.queryBy] != "undefined")
	...
	tinymce.create("tinymce.plugins.Mention", {
		init: function(ed) {
			...
			function prevCharIsSpace(l) {
				var start = ed.selection.getRng(true).startOffset,
					text = ed.selection.getRng(true).startContainer.data || "",
					charachter = text.substr(start >= l ? start - l : 0, 1);
				// console.log(charachter);
				return !!$.trim(charachter).length ? false : true;
			}

			ed.on("keypress", function(e) {
				var sel = ed.selection.getRng(true),
					fos = sel.startOffset,
					fns = sel.startContainer.data || "",
					dm = "",
					key = e.which || e.keyCode;

				if (fos < fns.length) {
					fns = fns.substr(0, fos);
				}

				var search_delimiter = fns.substr(fns.lastIndexOf(" ") + 1);

				//console.log(key);
				if (key == 8) {
					search_delimiter = search_delimiter.substring(
						0,
						search_delimiter.length - 1
					);
				} else {
					search_delimiter = search_delimiter + String.fromCharCode(key);
				}
				//console.log($(ed.selection.getNode()).text());
				...
				// );
				if (delimiterIndex > -1) {
					// if (delimiterIndex > -1 && prevCharIsSpace(search_delimiter.length)) {
					if (
						autoComplete === undefined ||
						(autoComplete.hasFocus !== undefined && !autoComplete.hasFocus)
					) {
						e.preventDefault();
						let matchDelimiter = autoCompleteData.delimiter[delimiterIndex];
						let startRag = fos - matchDelimiter.length + 1;
						let endRag = fos;
						let sContainer = sel.startContainer;
						let sel2 = sel.cloneRange();
						sel2.setStart(sContainer, startRag);
						sel2.setEnd(sContainer, endRag);
						ed.selection.setRng(sel2);

						// Clone options object and set the used delimiter.
						autoComplete = new AutoComplete(
							ed,
							$.extend({}, autoCompleteData, { delimiter: matchDelimiter })
						);
					}
				}
			});
