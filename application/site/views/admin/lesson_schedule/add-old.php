...
<div class="col-md-12 offset-1" id="app">
				...
				<?php 
                    ...
                    <div id="preItems">
						<div class="row addpre_option">
							...
							<select id="class_id"  name="class_id[1]"  class="form-select class_id" required>
						...
						<div class="mb-3 col-2 ">
                            ...
                            <select class="form-select "  data-id="1" id="section_id-1" name="section_id[1]" >
                       
							</select>
                        </div>
						<div class="col-md-2">
                        <label class="form-label">Lessons</label>
                        <select id="lesson_id" name="lesson_id[1]" class="form-select" required >
                            <option value="">Select</option>
							<?php 
							if(!empty($lessons)){
					...
					<a href="<?= base_url($_SESSION["login_session"]["url_slug"].'/lesson_schedule/'.$schools->id) ?>" class="btn btn-outline-primary">Cancel</a>
                </form>
...
var count = 1
	$('#addpre_option').click(function(){
		...
		$('#preItems').append('<div class="row addpre_option"> <div class="col-md-2"> <label class="form-label">Class Name</label> <select required id="class_id"   name="class_id['+count+']" class="form-select" > <option value="">Select</option> <?php  if(!empty($classes)){ foreach ($classes as $key => $value) { ?> <option value="<?=  $value->c_id ?>" data-id="'+count+'"><?=  $value->class_name ?></option> <?php   } }else{ echo ' <option>No classes found</option>'; } ?> </select> </div> <div class="mb-3 col-2 "> <label for="first_name" class="form-label">Section</label>  <select class="form-control " id="section_id-'+count+'" data-id="'+count+'" name="section_id['+count+']" > </select>  </div> <div class="col-md-2">  <label class="form-label">Lessons</label> <select id="lesson_id" required name="lesson_id['+count+']" class="form-select" > <option value="">Select</option> <?php  if(!empty($lessons)){ foreach ($lessons as $key => $value) { ?> <option value="<?=  $value->lesson_id ?>"><?=  $value->lesson_name ?></option> <?php  } }else{ echo ' <option>No classes found</option>'; } ?> </select> </div> <div class="col-md-2"> <label for="name" class="form-label">	From Date</label>  <input type="date" required name="from_date['+count+']" class="form-control" placeholder="Enter Option"  aria-label="" aria-describedby="basic-addon2">  </div> <div class="col-md-2"> <label for="name" class="form-label">	To Date</label>  <input type="date" required name="to_date['+count+']" class="form-control" placeholder="Enter Option"  aria-label="" aria-describedby="basic-addon2"> </div> <span class="col-md-2 m-t-25 "><button type="button" class=" btn  btn-danger" style="margin-top: 28px;" id="delpreoption"> <i class="bi bi-file-minus" ></i> </button></span> </div>');		
