...
class Comm_fun
{
	...
	public function is_logged_in()
	{
		...
		$is_logged_in = $this->CI->session->userdata('user_id');
	...
	function header_data(){
		...
		$id2 = $_SESSION['login_session']['user_id'];
		/* print_r($id2);exit; */
		$CI =& get_instance();
		$CI->load->model('Users_model');
		$res = $CI->Users_model->userInfo($id2);
		//print_r($res);exit;
		if($res['num']==1){
			$sd = $res['data']['color_code'];			
	...
	function send_mail($to,$subject,$message){
       ...
       $credentials = SendinBlue\Client\Configuration::getDefaultConfiguration()->setApiKey('api-key', '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');
        ...
        $sendSmtpEmail = new \SendinBlue\Client\Model\SendSmtpEmail([
             'subject' => $subject,
             'sender' => ['name' => 'RainbowFish', 'email' => 'no-reply@rainbowfishstudio.com'],
             'replyTo' => ['name' => 'RainbowFish', 'email' => 'no-reply@rainbowfishstudio.com'],
	...
	$mailgun = [			
	'apiKey' => '826aca90ff45534807e89e2445ffda0d-1b3a03f6-175dd82e',
	'domain' => 'mg.apexc.io',	
	];	   		
	$ch = curl_init();	
	...
	{
		$is_admin = $this->CI->session->userdata('user_role');
	...
	{
		$user_dept = strtolower($this->CI->session->userdata('user_department_name'));
