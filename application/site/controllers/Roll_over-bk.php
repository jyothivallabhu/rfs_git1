...
class Roll_over extends CI_Controller
{
	...
	public function save()
	{
		...
		if($this->input->post('academic_year')=='' ){
			...
			if(!empty($classes)){
				foreach ($classes as  $value) { 
					...
					if(!empty($next_classID)){
						$rolloverClassId = $next_classID['c_id'];
			...
			$ary['class_id'] = $this->input->post('class_id');
			$ary['section_id'] =$this->input->post('section_id'); 
			$res = $this->roll_over->studentInfo($ary);
			if ($res['num'] >= 1) {
				foreach($res['data'] as $val){
					$save_data = array(
						...
						'sd_academic_year'=>trim($_POST['roll_over_academic_year']),
						'sd_school_id'=>trim($_POST['school_id']),
						'sd_class_id'=>trim($_POST['class_id']),
						'sd_section_id'=>trim($_POST['section_id']),
						'added_by'=>$this->user_id,
					);
					
					$q = $this->db->insert('student_academic_details', $save_data);
				}
				$status = 1;
				$msg = '<div class="alert alert-success alert-dismissible" role="alert"> Added Successfully</div>';
				$url = base_url().'roll_over/add/'.$_POST['school_id'];
			}else{
				$msg = '<div class="alert alert-success alert-dismissible" role="alert">Failed</div>';
			}
			*/
		}
