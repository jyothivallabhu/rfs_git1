...
class Exam_types extends CI_Controller
{
	...
	public function index()
	{
		...
		/*total rows count*/
		$res = $this->admin->getExam_types($ary);
		if($res){			
			$totalRec = count($res);
		}else{
			$totalRec=0;
		...
		$config['base_url']    = base_url().'exam_types/ajaxPaginationData';
		...
		$posts = $this->admin->getExam_types($ary);
		...
		$data['main_content'] = 'admin/exam_types/exam_types_list';
	...
	function ajaxPaginationData(){
        ...
        //total rows count
		$data = $this->admin->getExam_types($conditions);
		...
		$users_list = $this->admin->getExam_types($conditions);
							...
							$html .='<td class="text-end">
								<div class="dropdown">
									...
									<div class="dropdown-menu dropdown-menu-end">
										...
										<a href="javascript:void(0);"  data-sid="'. $value->school_id .'"  data-did="'. $value->id .'" data-tbl="users" data-ctrl="exam_types"  class="dropdown-item text-danger delete_class">Delete</a>
									</div>
								</div>
							</td>';
							/* } */
	...
	public function add()
	{
		...
		$data['main_content'] = 'admin/exam_types/add';
		if(isset($_SESSION["login_session"]["school_id"]) && $_SESSION["login_session"]["school_id"] !=''){	
			$this->load->view('school_template/body', $data);
		}else{
			$this->load->view('dash_template/body',$data) ;
	...
	public function save()
	{
		...
		$rid = (isset($_SESSION['login_session']['school_id'])) ?  $this->url_slug.'/exam_types' :  'exam_types/'.$_POST["school_id"];
		...
		if($this->input->post('exam_name')=='' ){
			$msg = warning_msg('Please Enter exam name');	
		}else{
			$if_exists = $this->admin->check_if_term_exists($_POST);
			if ($if_exists != 0) {
				$status = 0;
				$msg= '<div class="alert alert-warning text-center alert-dismiss "> Exam Type already exists</div>';
			}else{
				$_POST['added_by'] = $this->user_id;
				
				$result = $this->db->insert('exam_types', $_POST);
				if($result){
					$status = 1;
	...
	{		
		...
		$rid = (isset($_SESSION['login_session']['school_id'])) ?  $this->url_slug.'/exam_types' :  'exam_types/'.$_POST["sid"];
		...
		if($id=="" || $res['num']!=1){
			...
			$result = $this->db->update('exam_types');
