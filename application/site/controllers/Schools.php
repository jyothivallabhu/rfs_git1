...
class Schools extends CI_Controller
{
	...
	public function index()
	{
		...
		/*total rows count*/
		$res = $this->admin->get_schools($ary);
		if($res){			
			$totalRec = count($res);
		}else{
			$totalRec=0;
		...
		$config['base_url']    = base_url().'schools/ajaxPaginationSchoolsData';
		...
		$posts = $this->admin->get_schools($ary);
		
		$data['pagination'] = $this->ajax_pagination->create_links();
		$data['showing_text'] = $this->ajax_pagination->showing_text;
		$data['schools'] = $posts;	
		
		$data['main_content'] = 'admin/schools/index';
	...
	public function add_school()
	{
		...
		$data['main_content'] = 'admin/schools/add_school';
		$this->load->view('dash_template/body', $data);
	}
	public function save_school()
	{
		...
		if($this->input->post('name')=='' ){
			...
			if ($if_exists > 0) {
				...
				$q = $this->db->insert('schools', $school_data);
				if($q){
					$status = 1;
	...
	public function edit_school($id){
		...
		$data['states'] = $this->admin->get_all_states();
		$data['main_content'] = 'admin/schools/edit_school';
		$this->load->view('dash_template/body', $data);
	}
